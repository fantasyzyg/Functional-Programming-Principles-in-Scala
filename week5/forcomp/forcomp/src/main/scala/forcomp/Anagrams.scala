...
object Anagrams extends AnagramsInterface {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = (w.toSeq groupBy (x => x.toLower) map (x => (x._1, x._2.length))).toList.sorted

  /** Converts a sentence into its character occurrence list. */
  def sentenceOccurrences(s: Sentence): Occurrences = {
    // 最简单的方法就是将 Sentence 转化为 String，然后调用上面的   wordOccurrences
  ...
  lazy val dictionaryByOccurrences: Map[Occurrences, List[Word]] =
    ((dictionary map (word => (wordOccurrences(word), word))) groupBy (n => n._1)).map(x => (x._1.sorted, x._2.map(y => y._2))).withDefaultValue(List())
  //  ((dictionary map (word => (wordOccurrences(word), word))) groupBy (n => n._1) mapValues (x => x map (y => y._2))).toMap withDefaultValue List()
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    occurrences match {
      ...
      case x :: xs => {
        ...
        }.toList ++ combinations(xs)
    }
  }

  /** Subtracts occurrence list `y` from occurrence list `x`.
   * // 前提条件是 y 已经是 x 的一个子集了
...
object Dictionary {
  ...
  def main(args: Array[String]): Unit = {
    ...
    println(dictionaryByOccurrences.values.filter(x => x.contains("tea")).toList)
    println(dictionaryByOccurrences.filter(x => x._2.contains("tea")).toList)
  }
}
